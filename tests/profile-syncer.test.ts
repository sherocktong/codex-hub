...
describe("profile-syncer", () => {
  ...
  it("syncs native profile preserving project nodes from config.toml", () => {
    ...
    expect(content).toContain('openai_base_url = "http://127.0.0.1:1234/v1"');
    // Re-syncing from an older config.toml that lacks the flag must still
    // disable the Responses-API-only remote compaction v2 protocol.
    expect(content).toContain("remote_compaction_v2 = false");
  });
  ...
  it("injects remote_compaction_v2 = false for qianwen profiles missing the flag", () => {
    const content = profileSyncer.buildFullProfileContent(
      "qwen-dev",
      { model: "qwen3.8-max-preview", provider: "qianwen" },
      "http://127.0.0.1:1234/v1",
      '[projects."/Users/kangtong/work"]\ntrust_level = "trusted"\n',
    );
    expect(content).toContain("[features]");
    expect(content).toContain("remote_compaction_v2 = false");
    expect(content.indexOf("[features]")).toBeLessThan(content.indexOf("remote_compaction_v2 = false"));
  });

  it("adds remote_compaction_v2 = false under an existing [features] section", () => {
    const content = profileSyncer.buildFullProfileContent(
      "qwen-dev",
      { model: "qwen3.8-max-preview", provider: "qianwen" },
      "http://127.0.0.1:1234/v1",
    ...
    const compactionIndex = content.indexOf("remote_compaction_v2 = false");
    const jsReplIndex = content.indexOf("js_repl = false");
    expect(compactionIndex).toBeGreaterThan(-1);
    expect(featuresIndex).toBeLessThan(compactionIndex);
    expect(compactionIndex).toBeLessThan(jsReplIndex);
  });

  it("overrides remote_compaction_v2 = true with false for chat-completions providers", () => {
    const content = profileSyncer.buildFullProfileContent(
      "kimi-dev",
      { model: "kimi-k2-5-coding", provider: "kimi" },
      "http://127.0.0.1:1234/v1",
    ...
    expect(content).toContain("remote_compaction_v2 = false");
    expect(content).not.toContain("remote_compaction_v2 = true");
  });
