...
describe("profile-syncer", () => {
  ...
  it("activateProfileConfig symlinks config.toml to the profile file", () => {
    const configPath = profileSyncer.CODEX_CONFIG_FILE;
    fs.mkdirSync(path.dirname(configPath), { recursive: true });
    ...
    expect(fs.readlinkSync(configPath)).toBe(profileSyncer.getNativeProfilePath("active"));
  });
  ...
  it("activateProfileConfig replaces an existing regular config.toml file with a symlink", () => {
    const configPath = profileSyncer.CODEX_CONFIG_FILE;
    fs.mkdirSync(path.dirname(configPath), { recursive: true });
    fs.writeFileSync(configPath, "theme = \"Catppuccin Latte\"\n", "utf-8");

    profileSyncer.activateProfileConfig("new", { provider: "kimi" }, "http://127.0.0.1:1234/v1");

    expect(fs.lstatSync(configPath).isSymbolicLink()).toBe(true);
    expect(fs.readlinkSync(configPath)).toBe(profileSyncer.getNativeProfilePath("new"));
  });

  it("deactivateProfileConfig removes the symlink only for the active profile", () => {
    const configPath = profileSyncer.CODEX_CONFIG_FILE;
    fs.mkdirSync(path.dirname(configPath), { recursive: true });
    const activeProfilePath = profileSyncer.getNativeProfilePath("active");
    fs.writeFileSync(activeProfilePath, "active", "utf-8");
    fs.symlinkSync(activeProfilePath, configPath);
