...
export function profileCommand(): Command {
  ...
  profile
    ...
    .action(safeAction(async (name: string, opts: ProfileAddOptions) => {
  ...
  profile
    ...
    .option("-u, --url <url>", "Base URL")
    .option("-p, --provider <provider>", `Provider type: ${VALID_PROVIDERS.join(" | ")}`)
    .action(safeAction(async (name: string, opts: ProfileUpdateOptions) => {
      ensureProfilesFile();
      const data = readJson<ProfilesData>(PROFILES_FILE);
      if (!data.profiles[name]) {
...
export function runCommand(): Command {
  return new Command("run")
    ...
    .action(safeAction(async (args: string[], _opts: Record<string, unknown>) => {
      ensureProfilesFile();
      const data = readJson<ProfilesData>(PROFILES_FILE);
